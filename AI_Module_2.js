...
function calculateMoveWeight(move, piece, currentCoord) {
   ...
   // 5. PRESERVE CYPHER CHESS SYSTEM OVERRIDES & GEOGRAPHIC PST VALUES
    ...
    if (targetPiece) {
        ...
        if (isCourtFlip || isBorderFlip) {
            score += 100000.0; // 👑 FIX: Elevate to ultimate priority domain
        }
    }

    // 🏛️ GEOGRAPHIC SECTOR EVALUATOR (The Missing PST Vector)
    if (simulatedActor.type === 'Pawn') {
        // Drive Pawns toward the Border (Row 5 is the sweet spot)
        if (targetSector === 'Border') score += 150.0;
        else if (targetRow === 4 || targetRow === 6) score += 50.0; // Pushing up the Field
    } else if (simulatedActor.type !== 'King' && simulatedActor.type !== 'Spy') {
        // Major Pieces (Rooks, Knights, Queens) get positioning bonuses for holding tactical high ground
        if (targetSector === 'Court') {
            score += 300.0; // Magnetized high ground for terminal check placement
        } else if (targetSector === 'Border') {
            score += 100.0; // Tactical staging gates
        }
    }
