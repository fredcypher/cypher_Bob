...
// --- MODULE 3: STATE MANDATE SEPARATION (CYPHER GEOMETRY RESOLUTION) ---
            ...
            for (const [coord, cell] of Object.entries(liveBoard)) {
                if (cell?.piece && cell.piece.color === 'white') {
                    ...
                    if (["Queen", "Rook", "Bishop", "Knight"].includes(cell.piece.type)) {
                        const attackerSector = getSectorType(coord); // Returns 'F', 'B', or 'C'
                        const kingSector = getSectorType(blackKingSq);
                        
                        if (attackerSector === 'F' && kingSector === 'F') {
    ...
    try {
        ...
        if (blackHistory.length >= 2) {
            const lastBlackMove = blackHistory[blackHistory.length - 1];      // E.g., i1 -> i2
            const priorBlackMove = blackHistory[blackHistory.length - 2];     // E.g., i2 -> i1
            ...
            if (
                ...
                fallbackStagnationFromSquare = lastBlackMove.to; // The coordinate it keeps returning to
            }
        }
    } catch (historyError) {
    ...
    const sortedPool = movePool.map(move => {
        try {
            ...
            if (activeStagnationPieceType) {
                // Gather all historical starting squares to find unmoved pieces
                const history = manager.state.counters?.moveHistory || [];
                const movedFromSquares = new Set(history.map(m => m.from));
                
                // Condition A: Is this a major back-row piece that hasn't moved yet?
                ...
                if (isMajorPiece && hasYetToMove) {
                    // Force absolute mobilization priority to develop back pieces
                    return { ...move, weight: 1000000.0 }; 
                } 
                
                // Condition B: Fallback if Condition A yields nothing — suppress the shuffling piece
                if (actorType === activeStagnationPieceType && move.from === fallbackStagnationFromSquare) {
                    return { ...move, weight: -1000000.0 }; // Force Bob to pick literally any other unit
                }
            }
            // =====================================================================
