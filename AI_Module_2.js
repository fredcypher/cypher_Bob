...
function calculateMoveWeight(actor, actorCoord, targetSquare) {
    ...
    if (targetPiece) {
        ...
        if (actor.type === 'Pawn') {
            score += 150; // Balanced capture premium (stops flank deviations)
        }
    }
    ...
    if (targetPiece) {
        ...
        if (isCourtFlip || isBorderFlip) {
            score += 3500; // Commands absolute critical prioritization
        }
    }

    // ----------------------------------------------------
    // 4. GEOGRAPHIC GEOMETRIC SYSTEM (Native Equator Alignment)
    // ----------------------------------------------------
    if (actor.type !== 'Queen' && actor.type !== 'King') {
        
        // --- PIECE-SPECIFIC GEOGRAPHY LAYER ---
        if (actor.type === 'Knight') {
            // Knights patrol perimeter of the Equator (Rows 4, 5, 6) to utilize ckm/ekm
            if (targetRow >= 4 && targetRow <= 6) {
                score += 150; 
            }

            // Supportive Escort: Reward Knight for clearing space down-row ahead of extant pawns
            for (let [coord, cell] of Object.entries(manager.boardIndex)) {
                if (cell && cell.piece && cell.piece.type === 'Pawn' && cell.piece.color === 'black') {
                    const pawnFileCode = coord.charAt(0).toLowerCase().charCodeAt(0);
                    const pawnRow = parseInt(coord.substring(1), 10);
                    const fileDist = Math.abs(targetFile.charCodeAt(0) - pawnFileCode);

                    if (fileDist <= 2 && targetRow < pawnRow && targetRow >= 3) {
                        score += 80; 
                    }
                }
            }
        } else if (actor.type !== 'Pawn') {
            // Standard Equator Anchoring for Rooks and Bishops (Replaces old playBobMove loop)
            const distToEquator = Math.abs(targetRow - 5);
            if (distToEquator === 0) {
                score += 200; 
            }
        }

        // Rule B1: Base sector value
        if (targetSector === 'Border' || targetSector === 'Court') {
            score += 60;
        }
    }

    // Rule B2 & B3: Pawn Runway Computations (Black pushes downwards towards Row 3)
    if (actor.type === 'Pawn') {
        const actorFile = actorCoord.charAt(0).toLowerCase();
        const actorRow = parseInt(actorCoord.substring(1), 10);

        if (targetFile === actorFile && targetRow < actorRow) {
            // Premium incentive for crossing down past the equator track
            if (targetRow <= 5 && targetRow >= 3) {
                score += 650; 
            }

            const centralFiles = ['c', 'd', 'e', 'f'];
            if (centralFiles.includes(targetFile) && targetRow >= 3) {
                score += 350; // Central runway premium
            } else if (targetRow >= 3) {
                score += 100; // Flank runway premium
            }
        }
    }

    // Secondary runway layer: Other assets supporting pathways ahead of active pawns
    for (let [coord, cell] of Object.entries(manager.boardIndex)) {
        if (cell && cell.piece && cell.piece.type === 'Pawn' && cell.piece.color === 'black') {
    ...
    // Rule B4: Line of sight threats directly to enemy King
    if (targetSector === 'Border' || targetSector === 'Court') {
        ...
        if (whiteKingCoord && typeof manager.isPathClear === 'function' && manager.isPathClear(targetSquare, whiteKingCoord)) {
            score += 150;
        }
    }
    ...
    // ----------------------------------------------------
    // 4. GEOGRAPHIC GEOMETRIC MINIMAX SYSTEM (Knight Optimized)
    // ----------------------------------------------------
    
    // Protect major pieces from inheriting blind structural positioning weights
    if (actor.type !== 'Queen' && actor.type !== 'King') {
        
        // --- PIECE-SPECIFIC GEOGRAPHY LAYER ---
        if (actor.type === 'Knight') {
            // Knights thrive on the perimeter of the Equator (Rows 4, 5, 6) to leverage ckm/ekm
            if (targetRow >= 4 && targetRow <= 6) {
                score += 500; // Stronger incentive than standard assets to patrol the defensive line
            }

            // DYNAMIC ESCORT: Reward the Knight for staying close to friendly Pawns to shepherd them
            for (let [coord, cell] of Object.entries(manager.boardIndex)) {
                if (cell && cell.piece && cell.piece.type === 'Pawn' && cell.piece.color === 'black') {
                    const pawnFileCode = coord.charAt(0).toLowerCase().charCodeAt(0);
                    const pawnRow = parseInt(coord.substring(1), 10);
                    ...
                    const fileDist = Math.abs(targetFile.charCodeAt(0) - pawnFileCode);
                    const rowDist = Math.abs(targetRow - pawnRow);

                    // If Knight is within a 2-square tactical envelope of an advancing friendly pawn
                    if (fileDist <= 2 && rowDist <= 2) {
                        score += 150; // Adds a supportive layer of protection
                    }
                }
            }
        } else if (actor.type !== 'Pawn') {
            // Standard Equator Anchoring for Rooks/Bishops
            const distToEquator = Math.abs(targetRow - 5);
            if (distToEquator === 0) {
                score += 400; 
            }
    ...
    // ----------------------------------------------------
    // 4. GEOGRAPHIC GEOMETRIC MINIMAX SYSTEM (Balanced)
    // ----------------------------------------------------
    ...
    if (actor.type !== 'Queen' && actor.type !== 'King') {
        ...
        // --- PIECE-SPECIFIC GEOGRAPHY LAYER ---
        if (actor.type === 'Knight') {
            // Knights patrol the tactical perimeter of the Equator (Rows 4, 5, 6)
            if (targetRow >= 4 && targetRow <= 6) {
                score += 150; // Calibrated down so it never drowns out active pawn pushes
            }

            // DYNAMIC ESCORT: Reward the Knight for anchoring *ahead* of advanced friendly pawns
            for (let [coord, cell] of Object.entries(manager.boardIndex)) {
                if (cell && cell.piece && cell.piece.type === 'Pawn' && cell.piece.color === 'black') {
                    const pawnFileCode = coord.charAt(0).toLowerCase().charCodeAt(0);
                    const pawnRow = parseInt(coord.substring(1), 10);
                    ...
                    // The Knight gains points only if it is occupying space DOWNSTREAM (lower row numbers) ahead of the pawn
                    if (fileDist <= 2 && targetRow < pawnRow && targetRow >= 3) {
                        score += 80; 
                    }
                }
            ...
            const distToEquator = Math.abs(targetRow - 5);
            if (distToEquator === 0) {
                score += 120; // Calibrated down to maintain global priority fluidity
            }
        }
    ...
    if (actor.type === 'Pawn') {
        ...
        // If pushing down forward on its own file
        if (targetFile === actorFile && targetRow < actorRow) {
            // Reward breaching down past the equator track
            if (targetRow <= 5 && targetRow >= 3) {
                score += 650; // Increased premium to guarantee the line of defense asserts itself
            }
    ...
    // Rule B4: Direct Threat pathing to enemy King
    if (targetSector === 'Border' || targetSector === 'Court') {
        const whiteKingCoord = manager.state.whiteKingCoord || Object.keys(manager.boardIndex).find(coord => {
            const p = manager.boardIndex[coord]?.piece;
            return p && p.type === 'King' && p.color === 'white';
    ...
    // ----------------------------------------------------
    // 4.5 STATIC TACTICAL SCANS (The Lookahead Illusion)
    // ----------------------------------------------------
    
    // Find the White King's coordinates for proximity tracking
    const whiteKingCoord = manager.state.whiteKingCoord || Object.keys(manager.boardIndex).find(coord => {
        const p = manager.boardIndex[coord]?.piece;
        return p && p.type === 'King' && p.color === 'white';
    });

    // 🕵️ SPY NO-FLY ZONE: Prevent Spies from stepping into a 2-square death radius near the King
    if (actor.type === 'Spy' && targetSector !== 'Court') {
        if (whiteKingCoord) {
            ...
            if (fileDist <= 2 && rowDist <= 2) {
                score -= 2500; // Absolute veto to prevent suicidal flips outside Court
                console.log(`⚠️ SPY ESCAPE VALVE: Refusing to let Spy step to ${targetSquare} near White King.`);
            }
        }
    }

    // 👑 COURT FLIP PROJECTED SHADOW: Protect Black King from instantaneous Court Spies
    if (actor.type === 'King' && targetSector === 'Court') {
        const vulnerableCourtSquares = [];

        // Scan the entire board using Bob Lite's native DOM boardIndex to locate active White Spies
        for (let [spyCoord, cell] of Object.entries(manager.boardIndex)) {
            if (cell && cell.piece && cell.piece.type === 'Spy' && cell.piece.color === 'white') {
                ...
                if (typeof manager.isPathClear === 'function') {
                    // Loop through every square in the board index to check lines of sight
                    for (let targetCoord of Object.keys(manager.boardIndex)) {
                        if (getSector(targetCoord) === 'Court') {
        ...
        // If the King's candidate target square falls inside a Spy's line-of-sight shadow
        if (vulnerableCourtSquares.includes(targetSquare)) {
            score -= 3000; // Hard veto penalty overriding all positional desires
            console.log(`🛡️ COURT FLIP SHADOW TRIPPED: Vetoing King move to ${targetSquare} due to projected Spy line.`);
        }
    }

    // 🛡️ SCAN 1: BLUNDER PREVENTION (Defensive Hanging Piece & Sniper Avoidance)
    if (actor.type === 'King' || actor.type === 'Queen' || actor.type === 'Rook') {
        // Safe check using Bob Lite's native threat analyzer tool
    ...
    // ⚔️ SCAN 2: DIRECT THREAT PREMIUM (Offensive Forcing Moves)
    for (let [coord, cell] of Object.entries(manager.boardIndex)) {
        if (cell && cell.piece && cell.piece.color === 'white') {
            const victim = cell.piece;

            if (victim.type === 'King' || victim.type === 'Queen' || victim.type === 'Rook') {
                // Look from the prospective targetSquare to see if we project threat rays at them
                if (typeof manager.isPathClear === 'function' && manager.isPathClear(targetSquare, coord)) {
                    if (victim.type === 'King') {
                        score += 700;  // High premium for delivering check pressure
                    } else if (victim.type === 'Queen' && actor.type !== 'Pawn') {
                        score += 250;  
                    }
                }
            }
